...
using Fgv.Acad.Financeiro.Tests.TestDatas;
...
namespace Fgv.Acad.Financeiro.Tests
{
    ...
    /// Provides methods to easily work with <see cref="FinanceiroDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<FinanceiroTestModule>
    {
        ...
        {
            void NormalizeDbContext(FinanceiroDbContext context)
            {
        ...
        protected void UsingDbContext(Action<FinanceiroDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FinanceiroDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FinanceiroDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FinanceiroDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FinanceiroDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FinanceiroDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FinanceiroDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FinanceiroDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FinanceiroDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FinanceiroDbContext, Task<T>> func)
        {
            T result;
