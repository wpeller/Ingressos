...
namespace Fgv.Acad.Financeiro.DataExporting.Excel.EpPlus
{
    public abstract class EpPlusExcelExporterBase : FinanceiroServiceBase, ITransientDependency
    {
        ...
        protected void AddHeader(ExcelWorksheet sheet, int startRowIndex, int columnIndex, params string[] headerTexts)
        {
            if (headerTexts.IsNullOrEmpty())
            {
                return;
            ...
            for (var i = 0; i < headerTexts.Length; i++)
            {
                AddHeader(sheet, startRowIndex, columnIndex, headerTexts[i]);
        ...
        protected void AddHeader(ExcelWorksheet sheet, int startRowIndex, int columnIndex, string headerText)
        {
            sheet.Cells[startRowIndex, columnIndex].Value = headerText;
            sheet.Cells[startRowIndex, columnIndex].Style.Font.Bold = true;
        }
